...
class TmdbController extends Controller
{
    public function autofill(Request $request)
    {
        ...
        foreach ($countries as $c) {
            if (! empty($c['name'])) {
                $countryNames[] = (string) $c['name'];
            }
            if (! empty($c['iso_3166_1'])) {
                $countryIso[] = (string) $c['iso_3166_1'];
            }
        }
        ...
        if ($type === 'tv') {
            if (isset($details['number_of_seasons']) && is_numeric($details['number_of_seasons'])) {
                $numberOfSeasons = (int) $details['number_of_seasons'];
            }
            if (isset($details['number_of_episodes']) && is_numeric($details['number_of_episodes'])) {
                $numberOfEpisodes = (int) $details['number_of_episodes'];
            }
            if ($seasonNumber > 0) {
                $seasonRes = Http::get($baseUrl."/tv/{$id}/season/{$seasonNumber}", [
                    'api_key' => $key,
                    'language' => 'fr-FR',
                ]);
                if ($seasonRes->ok()) {
                    $seasonJson = $seasonRes->json();
                    $episodesArr = is_array($seasonJson['episodes'] ?? null) ? $seasonJson['episodes'] : [];
                    $seasonEpisodeCount = count($episodesArr);
                }
            }
        }
