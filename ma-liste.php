...
// Récupérer les années disponibles pour les films de l'utilisateur
$annees = [];
$studios = [];
if (isset($_SESSION['user_id'])) {
    try {
        ...
        // Récupérer les années disponibles pour les films de l'utilisateur dans cette catégorie
        ...
        $stmtAnnees->execute([$user_id, $category]);
        ...
        $stmtStudios->execute([$user_id, $category]);
...
<body>
    ...
    <script>
        ...
        function loadStatistics(category) {
            ...
            xhr.open("GET", "./scripts-php/ma-liste-display/statistics.php?categorie=" + encodeURIComponent(category));
            xhr.onload = function() {
                if (xhr.status === 200) {
        ...
        function loadFilters(category) {
            var xhr = new XMLHttpRequest();
            xhr.open("GET", "./scripts-php/ma-liste-display/filters.php?categorie=" + encodeURIComponent(category));
            xhr.onload = function() {
                if (xhr.status === 200) {
                    var response = JSON.parse(xhr.responseText);
        ...
        function openTab(evt, category, page = 1) {
            ...
            Promise.all([
                fetch("./scripts-php/ma-liste-display/filters.php?categorie=" + encodeURIComponent(category))
                    .then(response => response.json()),
                fetch("./scripts-php/ma-liste-display/statistics.php?categorie=" + encodeURIComponent(category))
                    .then(response => response.json()),
                fetch("./scripts-php/ma-liste-display/display.php?categorie=" + encodeURIComponent(category) + "&page=" + encodeURIComponent(page))
                ...
                // Déclencher l'événement pour réattacher les événements
    ...
    <script src="./scripts-js/ma-liste-actions.js" defer></script>
